...
description: |
  Use when a plan contains UI components, user interactions, or visual design
  elements and needs review from a designer's perspective.
---

## Operating Context

- Origin: adapted from gstack
- 디스패치 주체: plan-reviewer 또는 메인 LLM
- 리뷰 대상: plan files
...
### Visual Mockup Tool (optional)

If a visual mockup tool is available, it can generate real visual mockups from design
briefs. Use it by default when available, not as an afterthought.

**The rule is simple:** If the plan has UI and a mockup tool is available, generate mockups.
...
If no visual mockup tool is available, proceed with text-only review using ASCII
wireframes or HTML sketches as alternatives.
...
* What prior design reviews exist? (check git log)
...
Design artifacts (wireframes, comparison boards, sketches) should be saved to
`docs/designs/`, `/tmp/`, or any project-local directory. Design artifacts are USER
data, not project files. They persist across branches, conversations, and workspaces.

Use `open file://...` to open HTML wireframes or comparison boards in the user's default browser.
...
## Step 0.5: Visual Design Communication

If the plan involves UI — screens, pages, components, visual changes — create
visual representations to ground the design review in concrete artifacts.

### Default: Text-Based Wireframes

For each key screen/interaction in scope, create **ASCII wireframes** or
**HTML sketch files** (saved to `docs/designs/`):

1. Construct a design brief from the plan's description (and DESIGN.md if present)
2. Create ASCII wireframes showing layout, hierarchy, and key interactions
3. For complex UIs, write a simple HTML file with inline CSS as a static mockup
4. Use `open file://...` to show HTML mockups in the user's browser

**Generate wireframes ONE AT A TIME.** The inline review flow benefits from
sequential control.

### Feedback Loop

Present each wireframe to the user and use AskUserQuestion:

"Here's the wireframe for [screen/component]. Key design decisions:
- [decision 1]
- [decision 2]
...
YOUR NOTES: [key changes requested]
Is this right?"

Note which direction was approved. This becomes the reference for all subsequent review passes.

**Multiple screens:** If the plan covers multiple UI areas, wireframe ALL key screens
and get user approval before starting the 7-pass review.

## Design Outside Voices (parallel)

Read `references/outside-voices-design.md` and follow the instructions.
The reference contains: Codex availability check, Claude design subagent prompt,
litmus scorecard synthesis, and pass integration rules.
...
If a visual mockup tool is available AND a dimension rates below 7/10,
...
If no mockup tool is available, continue with text-based descriptions of what
10/10 looks like.
...
If `docs/solutions/` 디렉토리에 관련 학습 문서가 있으면 참고하여 분석에 반영한다. 과거 학습과 일치하는 리뷰 발견 시:

**"Prior learning applied: [key] (from [source])"**

This makes the compounding visible. The user should see that the review process is getting
...
FIX TO 10: If DESIGN.md exists, annotate with specific tokens/components. If no DESIGN.md, flag the gap and proceed with universal design principles.
...
If yes, regenerate with feedback summarizing the changes or generate new variants with an updated brief.
...
* **NEVER use AskUserQuestion to ask which variant the user prefers.** Always create a comparison board first and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use AskUserQuestion ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
After completing the review, update the **plan file** so review status is visible.

### Detect the plan file

1. Check if there is an active plan file in this conversation.
2. If not found, skip this section silently.

### Generate the report

Using the results from your own review above, produce this table:
...
| Design Review | `/plan-design-review` | {this run} | {status} | score: {initial}/10 → {final}/10, {decisions} decisions |
| DX Review | `/plan-devex-review` | — | — | — |
```

Below the table:
- **UNRESOLVED:** total unresolved decisions from this review
- **VERDICT:** current review status
...
2. If an existing `## REVIEW REPORT` section exists, **replace it** entirely.
3. Append (or replace) the generated `## REVIEW REPORT` section as the **last section**.

After writing, verify: the file's last `## ` heading is `## REVIEW REPORT`.

## Next Steps — Review Chaining

Recommend the next review(s) based on what this design review discovered.
...
Read `references/exit-plan-mode-gate.md` and execute the self-check.
Do NOT call ExitPlanMode until all checks pass.
