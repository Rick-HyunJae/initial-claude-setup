...
description: |
  Use when a plan involves developer-facing APIs, CLIs, SDKs, documentation,
  or developer tooling and needs review from a developer experience perspective.
---
...
## Prerequisite: Design Doc Check

When the design doc check above prints "No design doc found," inform the user:

> "이 브랜치에 디자인 문서가 없습니다. 디자인 문서가 있으면 더 정확한 리뷰가 가능합니다.
> 디자인 문서 없이도 범용 원칙 기반으로 리뷰를 진행할 수 있습니다."

Options:
- A) 디자인 문서 없이 표준 리뷰 진행
- B) 리뷰 중단 — 디자인 문서 작성 후 재시작

If they choose A: proceed with standard review using universal design principles.
If they choose B: exit and let user prepare the design doc first.
...
4. Load Hall of Fame reference for this pass (read relevant section from references/dx-hall-of-fame.md)
...
This is NOT a scored pass. It's a checklist of proven DX patterns from established projects.
...
| DX Review | `/plan-devex-review` | {this run} | {status} | score: {initial}/10 → {final}/10, TTHW: {current} → {target} |
```
...
Recommend next reviews based on what this DX review discovered:
