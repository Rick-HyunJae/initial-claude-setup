...
Project: DOUZONE Data Visualization Editor (TypeScript/React/ProseMirror/TipTap)
...
- **module**: Module or area affected (e.g. `packages/customize/extension-table`, `context/editorManager`)
- **date**: ISO date in `YYYY-MM-DD`
- **problem_type**: One of the values listed in the Tracks table above
- **component**: One of `prosemirror_core`, `tiptap_core`, `tiptap_extension`, `editor_config`, `editor_context`, `media_context`, `webview_context`, `react_component`, `custom_hook`, `mobile_editor`, `build_config`, `development_workflow`, `testing_framework`, `documentation`, `tooling`
...
- **root_cause**: One of `missing_import`, `missing_index`, `wrong_api`, `scope_issue`, `async_issue`, `async_timing`, `memory_leak`, `config_error`, `logic_error`, `type_error`, `test_isolation`, `missing_validation`, `missing_permission`, `missing_workflow_step`, `inadequate_documentation`, `missing_tooling`, `incomplete_setup`
...
| Component | 위치 / 설명 |
|---|---|
| `prosemirror_core` | ProseMirror schema, state, transform, view, plugin |
| `tiptap_core` | TipTap 코어 (Editor, Extension 베이스) |
| `tiptap_extension` | 커스텀 Extension (`packages/customize`) |
| `editor_config` | 에디터 설정 (`config/`) |
| `editor_context` | 에디터 컨텍스트 매니저 (`context/editorManager`) |
| `media_context` | 미디어 컨텍스트 (`context/media`) |
| `webview_context` | WebView 컨텍스트 |
| `react_component` | 공통 UI 컴포넌트 (`components/`) |
| `custom_hook` | 커스텀 React 훅 (`hooks/`) |
| `mobile_editor` | 모바일 에디터 패키지 |
| `build_config` | Webpack/DLL 빌드 설정 |
| `development_workflow` | 개발 워크플로우 |
| `testing_framework` | 테스트 프레임워크 |
| `documentation` | 문서화 |
| `tooling` | 개발 도구 |
...
Example — before (breaks strict YAML):
    ...
    symptoms:
      - "`TipTap` extension transaction dispatch failed silently"
