...
description: |
  Use when reviewing a plan from a CEO/founder perspective — rethinking
  the problem from first principles, challenging premises, and evaluating
  strategic alignment and ambition level.
---
...
> "No design doc found for this branch. A structured problem statement, premise
> challenge, and explored alternatives would give this review much sharper input
> to work with. The design doc is per-feature, not per-product — it captures the
> thinking behind this specific change."

Options:
- A) 사용자에게 직접 질문하여 추가 컨텍스트 수집
- B) Skip — proceed with standard review

If they skip: "No worries — standard review." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Ask the user directly about the problem statement, constraints, and chosen approach.
Use the answers as context for the review. Then proceed with the next step below.

**Mid-session detection:** During Step 0A (Premise Challenge), if the user can't
articulate the problem, keeps changing the problem statement, answers with "I'm not
sure," or is clearly exploring rather than reviewing — offer to collect more context:

> "It sounds like you're still figuring out what to build — that's totally fine.
> Want me to ask you some structured questions to clarify the problem space?"

Options: A) Yes, ask me structured questions. B) No, keep going.

If they keep going, proceed normally — no guilt, no re-asking.

If they choose A: Ask structured questions about the problem statement, constraints,
and explored alternatives. Use the answers as context and resume the review.
...
```
NOTE: These represent human-team implementation hours. With AI-assisted coding,
...
Read `references/outside-voices-plan-challenge.md` and follow the instructions.
The reference contains: Codex availability check, independent review prompt,
cross-model consensus handling, and fallback to Claude subagent.
...
각 TODO는 다음 형식으로 AskUserQuestion을 통해 개별 제시:
- **제목**: 한 줄 요약
- **우선순위**: P0/P1/P2
- **근거**: 리뷰에서 발견된 이유
선택지: A) TODOS.md에 추가 B) 건너뛰기 C) 이번 PR에서 바로 구현
...
|--------|---------|------|--------|----------|
| CEO Review | `/plan-ceo-review` | {this run} | {status} | {summary from your Completion Summary} |
...
- **VERDICT:** current review status (e.g., "CEO CLEARED — ready for eng review")
...
2. If an existing `## REVIEW REPORT` section exists, **replace it** entirely. Do NOT allow
   prior versions to leave the report mid-file when an older report already lived
   there — the user then sees a plan whose review report is not at the bottom and
   assumes it's stale.
3. Append (or replace) the generated `## REVIEW REPORT` section as the **last section** of
   the plan file.
...
Recommend the next review(s) based on what this CEO review discovered.
